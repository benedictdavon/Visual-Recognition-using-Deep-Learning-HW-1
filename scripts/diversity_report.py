...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Rank ensemble candidates by diversity versus an anchor.")
    parser.add_argument("--manifest", type=str, required=True, help="YAML manifest describing anchor and candidates.")
    parser.add_argument("--output-dir", type=str, default="outputs/diversity")
    parser.add_argument("--anchor", type=str, default=None, help="Override anchor candidate name from manifest.")
    parser.add_argument("--metric", type=str, default=None, help="Validation metric used for gap filtering.")
...
def _aligned_indices(anchor_ids: np.ndarray, candidate_ids: np.ndarray) -> np.ndarray:
    ...
    try:
        indices = np.array([candidate_lookup[sample_id] for sample_id in anchor_ids.tolist()], dtype=np.int64)
...
def _js_divergence(anchor_probs: np.ndarray | None, candidate_probs: np.ndarray | None) -> float | None:
...
def _normalize_branch(value: Any) -> str | None:
    ...
    if branch not in _VALID_BRANCHES:
        raise ValueError(f"Unsupported branch value '{value}'. Expected one of: {sorted(_VALID_BRANCHES)}")
    return branch
...
def main() -> int:
    ...
    treat_branch_variants_as_distinct = bool(manifest.get("treat_branch_variants_as_distinct", False))
    ...
    for entry in candidates:
        ...
        if val_metrics_path is None or val_artifact_path is None:
            raise ValueError(f"Candidate '{entry['name']}' requires both 'val_metrics' and 'val_artifact'.")
        ...
        test_artifact = load_artifact(test_artifact_path) if test_artifact_path is not None else None
    ...
    for entry in loaded_candidates:
        ...
        candidate_val_targets = entry["val_artifact"].targets[val_indices] if entry["val_artifact"].targets is not None else None
        if anchor_val.targets is None and candidate_val_targets is None:
            raise ValueError("Val artifacts must contain ground-truth targets for rescue-count computation.")
        val_targets = anchor_val.targets if anchor_val.targets is not None else candidate_val_targets
        ...
        candidate_val_probs = (
            entry["val_artifact"].probs[val_indices] if entry["val_artifact"].probs is not None else None
        )
        val_disagreement = float(np.mean(anchor_val.preds != candidate_val_preds))
        rescue_count = int(np.sum((anchor_val.preds != val_targets) & (candidate_val_preds == val_targets)))
        ...
        if anchor_test is not None and entry["test_artifact"] is not None:
            test_indices = _aligned_indices(anchor_test.sample_ids, entry["test_artifact"].sample_ids)
            candidate_test_preds = entry["test_artifact"].preds[test_indices]
            candidate_test_probs = (
                entry["test_artifact"].probs[test_indices] if entry["test_artifact"].probs is not None else None
            )
        ...
        if entry["name"] != anchor_name and test_disagreement is not None and test_disagreement < min_test_disagreement:
        ...
        if entry["name"] != anchor_name and val_js_divergence is not None and val_js_divergence < min_js_divergence:
    ...
    report = {
        ...
        "continued_probe_candidates": ranked_df.loc[ranked_df["probe_status"] == "continue_probe", "name"].tolist(),
        "stopped_probe_candidates": ranked_df.loc[ranked_df["probe_status"] == "stop_probe", "name"].tolist(),
