...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for queued validation jobs."""
...
def _build_command(python_exe: str, default_config: str, job_cfg: dict) -> list[str]:
    ...
    if ckpt:
        ...
        raise ValueError(
            "Each validation job needs either 'ckpt' or both 'run_dir' and 'checkpoint_name'."
        )
...
def main() -> int:
    """Execute validation jobs from a batch YAML manifest."""
    args = parse_args()
