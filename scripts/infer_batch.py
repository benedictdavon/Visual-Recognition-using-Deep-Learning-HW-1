"""Run multiple inference jobs sequentially from a batch YAML config."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Python executable used to launch scripts/infer.py.",
    )
...
def _default_output_dir(job_name: str) -> str:
    return str((ensure_dir(ROOT / "outputs" / "infer_batch") / job_name).resolve())


def _build_command(python_exe: str, default_config: str, job_cfg: dict) -> list[str]:
    cmd = [python_exe, str((ROOT / "scripts" / "infer.py").resolve())]
    ...
    if bool(job_cfg.get("use_ema", False)):
        cmd.append("--use-ema")
    if bool(job_cfg.get("tta", False)):
        cmd.append("--tta")
    if bool(job_cfg.get("save_probs", False)):
        cmd.append("--save-probs")
    if bool(job_cfg.get("save_logits", False)):
...
def main() -> int:
    ...
    print("\nAll queued inference jobs finished successfully.")
    return 0
